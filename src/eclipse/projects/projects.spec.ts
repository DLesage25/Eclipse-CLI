...
const mockComponentSelectionPromptMock = jest.fn();
jest.mock(
    '../prompts/componentSelection.prompt.ts',
    () => () => mockComponentSelectionPromptMock()
);

const mockEnvironmentSelectionPromptMock = jest.fn();
jest.mock(
    '../prompts/environmentSelection.prompt.ts',
    () => () => mockEnvironmentSelectionPromptMock()
);
...
describe('projects', () => {
    ...
    describe('viewProjectSecrets', () => {
        ...
        it('should return early if no secrets are found and log a warning', async () => {
            ...
            await projects.viewProjectSecrets(
                { _id: '123' } as Project,
                'component',
                'environment'
            );
    ...
    describe('printSecrets', () => {
        ...
        it('should return early if no secrets are found and log a warning', async () => {
            mockComponentSelectionPromptMock.mockResolvedValue('component');
            mockEnvironmentSelectionPromptMock.mockResolvedValueOnce(
                'environment'
            );
            jest.spyOn(secretsMock, 'getPartialSecrets');
            jest.spyOn(loggerMock, 'warning');

            await projects['printSecrets']({
                _id: '123',
                secrets: [
                    {
                        id: 1,
        ...
        it('should print all formatted secrets if found', async () => {
            mockComponentSelectionPromptMock.mockResolvedValueOnce('component');
            mockEnvironmentSelectionPromptMock.mockResolvedValueOnce({
                environment: 'environment',
            });
    ...
    describe('injectLocalProjectSecrets', () => {
        ...
        it('should initialize shell with no secrets if none found', async () => {
            ...
            await projects.injectLocalProjectSecrets(
                'npm',
                ['start'],
                'component',
                'environment'
